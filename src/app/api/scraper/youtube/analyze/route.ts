...
2. 무관한 영상 처리: 단순 기기 리뷰나 타 IT 업계 이슈 등 당사 핵심 키워드(통신망, 플랫폼 규제 등)와 관련성이 극히 낮은 영상은 억지로 리스크를 부여하지 말고, riskLevel을 "하", isRelated를 false로 설정한 뒤 핵심 내용만 간결하게(2~3문장 이내) 요약하세요.
3. 관련 영상 처리: 당사와 직/간접적 연관성이 있는 경우에만, 임원진이 빠르게 파악할 수 있도록 불필요한 미사여구를 빼고 [비즈니스 리포트] 형식으로 아래 3개 항목을 간결하고 명확하게 도식화하여 작성하세요.
   - ■ 영상 내용 및 핵심 여론 (주요 사실관계 및 댓글 민심 요약)
   - ■ 당사 연관성 (팩트에 기반하여 당사 비즈니스나 통신 업계에 미칠 객관적 영향력)
   - ■ SKB 진단 및 제언 (구체적이고 실현 가능한 대응 방향 1~2줄)
