...
2. **여론/감정 왜곡 절대 금지**: 네티즌이 기술적 한계나 개인적인 경험을 단순 언급하거나 오히려 국내 통신사/서비스를 옹호하는 내용임에도 불구하고, 이를 "통신사에 대한 불만이나 비판"으로 과장하여 해석하지 마세요. 문맥의 실제 뉘앙스(중립, 옹호, 단순 사실 나열)를 정확하고 객관적으로 반영하세요.
3. 출력 양식 엄수: 해당 커뮤니티 글이 통신사/플랫폼 이슈와 관련이 있든, 전혀 무관한 단순 유머/일반 글이든 상관없이 **무조건 아래 3가지 카테고리(■) 양식을 예외 없이 고정하여 작성하세요.**
...
4. 무관한 일반글 내용 시 리스크 레벨: 당사와 무관한 단순 잡담 등으로 판별될 경우, riskLevel은 무조건 "하", isRelated는 false로 반환하고, 위 양식 중 2, 3번 항목은 1줄 이내로 극히 짧게 끊어내세요.
  ...
  "isRelated": true 또는 false
        ...
        try {
            ...
            if (apiError.status === 503 || JSON.stringify(apiError).includes('503') || JSON.stringify(apiError).includes('high demand')) {
                return NextResponse.json({ error: '구글 AI 서버 트래픽 폭주로 일시 지연(503) 중입니다. 1~2분 뒤 다시 시도해주세요.' }, { status: 503 });
            }
            throw apiError;
        }
